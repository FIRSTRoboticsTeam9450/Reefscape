...
//     private int tid; -> ID of the tag
//     private boolean up = false;
//     private Pose2d currentPose; -> current x, y and rotation of the robot
...
//         stuckCounter = 0;
//         redAlliance = DriverStation.getAlliance().get() == Alliance.Red; -> check which alliance side its on
//         currentPose = drive.getState().Pose;

//         // grab target tag from limelight to align to
//         int tid = (int)LimelightHelpers.getFiducialID("limelight-coral"); -> get tag id
//         this.tid = tid;
...
//             double[] pose = getAlignPos(map.get(tid), Constants.AlignOffsets.firstCoralBack); -> first approach to the reef
//             pidX.setSetpoint(pose[0]);
//             pidY.setSetpoint(pose[1]);
//             pidRotate.setSetpoint(pose[2]); -> set pid locations
//         } else {
//             hasTarget = false;
//         }
//         up = false;
...
//      */
//     private double[] getAlignPos(double[] targetPos, double tagForwardOffset) { -> target pose is from the map of tags
//         double tagLeftOffset = Constants.AlignOffsets.leftReef;
//         if (position == AlignPos.RIGHT) { -> Check which side of the tag to go
//             tagLeftOffset = Constants.AlignOffsets.rightReef;
//         }

//         // Algae
//         if (position == AlignPos.CENTER || score.getPos() == ScoringPos.ALGAEL1 || score.getPos() == ScoringPos.ALGAEL2) { -> if going to algae
...
//         if (rotation < -Math.PI) { -> if rotation ends up higher than pi, rotate the other direction because its shorter
//             rotation += 2 * Math.PI;
...
//         if (position == AlignPos.RIGHT && LimelightHelpers.getFiducialID("limelight-coral") != tid) { -> only run vision if can see tag
...
//                 double[] pose = getAlignPos(map.get(tid), Constants.AlignOffsets.scoreCoralBack); -> Approach to score I think needs this because can't see the tag
//                 pidX.setSetpoint(pose[0]); -> x value
//                 pidY.setSetpoint(pose[1]); -> y value
//                 pidRotate.setSetpoint(pose[2]); -> rotation value
//             }
...
//             double powerX = pidX.calculate(currentPose.getX()); -> let pid calculate power based on your current x vs the target x
...
//             powerX += .05*Math.signum(powerX); -> Increase power by .05 no matter what sign
...
//             Logger.recordOutput("Reefscape/Align/rot error", pidRotate.getError()); -> logging
...
//             if (redAlliance) {
//                 powerX *= -1; -> switch direction if it is red alliance
//                 powerY *= -1;
//             }
...
//             SwerveRequest request = driveRequest.withVelocityX(powerX).withVelocityY(powerY).withRotationalRate(powerRotate); -> make it drive!!!
